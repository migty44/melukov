...
function createAWithHref(hrefValue) {
    ...
    alink.setAttribute('href', hrefValue);
...
function deleteTextNodes(where) {
    ...
    for (var i = 0; i < where.childNodes.length; i++) {
        allChilds.push(where.childNodes[i]);
    }

    for (var l = 0; l < allChilds.length; l++) {
        if (allChilds[l].nodeType == 3) {
            where.removeChild(allChilds[l]);
        }
    }
}
...
function deleteTextNodesRecursive(where) {
    ...
    for (var l = 0; l < allChilds.length; l++) {
        ...
        if (allChilds[l].childNodes.length > 0) {
            deleteTextNodesRecursive(allChilds[l]);
        }
    }
}
...
function collectDOMStat(root) {
    ...
    NewObject = {
        ...
        getNodeClass: function(arr) {
            return this.name;
        },
        getNodeTag: function(arr) {
            return this.name;
        },
        nodeType333: 0
    } 
    ...
    for (var i = 0; i < root.childNodes.length; i++) {
        ...
        if (root.childNodes[i].nodeType == 3) {
            ...
            nodeClass.push(root.childNodes[i].classList);
            nodeTagArr.push(root.childNodes[i].tagName);
        }
    }
}
